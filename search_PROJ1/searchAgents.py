...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Calculate the Manhattan distances from Pacman's current position to each unvisited corner
    distances = [util.manhattanDistance(position, corner) for corner in unvisited_corners]

    # Return the minimum distance as the heuristic estimate
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foods:
        return 0  # No food left, heuristic value is 0

    # Calculate Manhattan distances to all remaining food pellets
    distances = [util.manhattanDistance(position, food) for food in foods]

    # Return the sum of minimum distances as the heuristic estimate
    return sum(distances) / len(foods)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        # Check if there's food at the current state
