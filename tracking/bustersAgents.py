...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = [
            ...
            if livingGhosts[i + 1]  # i+1 because ghost indices start from 1 in gameState
        ]
        ghostPositions = []
        for ghostBelief in livingGhostPositionDistributions:
            mostLikelyPosition = ghostBelief.argMax()
            ghostPositions.append(mostLikelyPosition)
        # Choose action that minimizes the maze distance to the closest ghost
        best_action = None
        min_dis = float('inf')
