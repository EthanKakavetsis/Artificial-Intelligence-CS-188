...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Define domains for Pacman and Ghosts
    ...
    # Calculate true Manhattan distances and add noise for OBS0 and OBS1
    obs0_domain = set()
    obs1_domain = set()
    
    for pac_pos in variableDomainsDict[PAC]:
        for ghost0_pos in variableDomainsDict[GHOST0]:
            true_dist0 = manhattanDistance(pac_pos, ghost0_pos)
            for noise in range(-MAX_NOISE, MAX_NOISE + 1):
                noisy_dist0 = true_dist0 + noise
                if noisy_dist0 >= 0:
                    obs0_domain.add(noisy_dist0)
            
        for ghost1_pos in variableDomainsDict[GHOST1]:
            true_dist1 = manhattanDistance(pac_pos, ghost1_pos)
            for noise in range(-MAX_NOISE, MAX_NOISE + 1):
                noisy_dist1 = true_dist1 + noise
                if noisy_dist1 >= 0:
                    obs1_domain.add(noisy_dist1)
    
    # Convert sets to sorted lists and remove duplicates
    variableDomainsDict[OBS0] = sorted(list(obs0_domain))
    variableDomainsDict[OBS1] = sorted(list(obs1_domain))
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            if noisyDistance is None:
                return 1.0  # P(None | ghost in jail) = 1
            else:
                return 0.0  # P(noisyDistance | ghost in jail) = 0 for all noisyDistance != None
        else:
            # Normal case: Ghost is not in jail
            trueDistance = manhattanDistance(pacmanPosition, ghostPosition)
            if noisyDistance is None:
                return 0.0  # If noisyDistance is None and ghost is not in jail, probability should be 0
            else:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            prob = self.getObservationProb(observation, pac_pos, ghostPosition, jail_pos)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        updated_belief = DiscreteDistribution()
        
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, prob in newPosDist.items():
                updated_belief[newPos] += self.beliefs[oldPos] * prob
        
        self.beliefs = updated_belief
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            prob = self.getObservationProb(observation, pacmanPosition, particle, jail_pos)
            belief_dis[particle] += prob
        # Check if all particles have zero weight
        if belief_dis.total() == 0:
            self.initializeUniformly(gameState)
        else:
            # Resample particles based on their weights
            self.particles = [belief_dis.sample() for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            newparticle.append(new_position)
